...
**Base URL:** `/tickets` (hoặc `/api/v1/tickets` nếu có prefix)

**Authentication:** Bearer Token (JWT)

**Note:** Frontend service sử dụng base URL `/tickets`. Backend có thể có prefix `/api/v1`.
  ...
  "items": [
    {
      ...
      "description": "After latest update, getting error",
...
Tương tự như Get All Tickets: `pageNumber`, `pageSize`, `sortBy`, `sortDescending`, `search`

#### Response

Trả về `PaginatedResponse<Ticket>` giống như endpoint Get All Tickets.
...
[
  {
    ...
    "slaId": "sla-guid",
    "slaName": "Premium Support",
    "dueDate": "2026-01-18T08:00:00Z",
    "firstResponseDate": "2026-01-18T06:15:00Z",
    "resolvedDate": null,
    "closedDate": null,
    "satisfactionRating": null,
    "satisfactionComment": null,
...
Trả về object `Ticket` trực tiếp:
...
| `customerId` | string (Guid) | No | ID customer |
| `contactId` | string (Guid) | No | ID contact báo cáo |
| `type` | number (0-4) | No | Loại ticket: 0=Question, 1=Problem, 2=Incident, 3=FeatureRequest, 4=Task |
| `priority` | number (0-3) | No | Độ ưu tiên: 0=Low, 1=Medium, 2=High, 3=Critical |
| `channel` | string | No | Kênh tiếp nhận (Email, Web, Phone, Chat, Social Media, API) |
| `slaId` | string (Guid) | No | ID SLA áp dụng |
| `assignedToUserId` | string (Guid) | No | ID agent xử lý |
| `tags` | string | No | Tags JSON array format: `"[\"tag1\", \"tag2\"]"` |
| `dueDate` | string (ISO 8601) | No | Due date (auto tính nếu có SLA) |

#### Response (201 Created)

Trả về object `Ticket` đầy đủ:
...
Trả về object `Ticket` đã cập nhật (đầy đủ 28 fields).
...
Trả về object `Ticket` đã cập nhật với `assignedToUserId` và `assignedToUserName` mới.

---

### 8. Resolve Ticket

Đánh dấu ticket đã giải quyết. Status chuyển sang `Resolved` (5).
...
Không có response body. HTTP Status 200 OK.

---

### 9. Close Ticket

Đóng ticket. Status chuyển sang `Closed` (6).
...
**Note:** Backend có thể trả về thông tin tổng thời gian pause nếu cần.
```
...
Leo thang ticket lên level cao hơn. Tăng priority và `escalationCount`.
...
**Note:** Priority sẽ tăng lên 1 bậc (Medium → High → Critical). Nếu đã là Critical thì giữ nguyên.
```
...
Không có response body. HTTP Status 204 No Content.
...
```bash
curl -X POST "http://localhost:3000/tickets/{ticket-id}/pause-sla" \
  -H "Authorization: Bearer {token}" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST "http://localhost:3000/tickets/{ticket-id}/resume-sla" \
  -H "Authorization: Bearer {token}"
```
...
```bash
curl -X POST "http://localhost:3000/tickets/{ticket-id}/resolve" \
  -H "Authorization: Bearer {token}"
```
...
```bash
curl -X POST "http://localhost:3000/tickets/{ticket-id}/close" \
  -H "Authorization: Bearer {token}"
```
...
const API_BASE_URL = 'http://localhost:3000'; // hoặc backend URL của bạn
...
export class TicketService {
  ...
  async getTickets(params: {
    ...
    return response.data; // PaginatedResponse<Ticket>
  }
  
  // Lấy tickets của tôi
  async getMyTickets(params?: { pageNumber?: number; pageSize?: number }) {
...
export default function TicketList() {
  ...
  async function loadTickets() {
    try {
      ...
      setTickets(data.items); // Direct access, no wrapper
    } catch (error) {
