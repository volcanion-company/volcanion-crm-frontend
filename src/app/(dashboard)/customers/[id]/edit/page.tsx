...
import { CustomerType, CustomerStatus, CustomerSource, UpdateCustomerRequest } from '@/types';
...
export default function EditCustomerPage() {
  ...
  const [initialType, setInitialType] = useState<CustomerType | null>(null);

  // Pre-populate form data when customer is loaded
  useEffect(() => {
    if (customer) {
      setFormData({
        name: customer.name || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (initialType !== null && formData.type !== undefined && formData.type !== initialType) {
      const typeChanged = confirm(
        `${t('edit.typeChangeWarning')} ${tCommon('continue')}?`
      );
  ...
  if (!customer) {
    return (
      <div className="text-center py-12">
        ...
        <Link href="/customers">
          <Button className="mt-4">{tCommon('back')}</Button>
        </Link>
      </div>
    );
  }
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  {initialType !== null && formData.type !== undefined && formData.type !== initialType && (
                    <p className="text-xs text-orange-600 mt-1">
                      ⚠️ {t('edit.typeChangeWarning')}
                    </p>
                  )}
                </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Nhập User ID (Guid format). Để trống nếu không thay đổi.
                </p>
              </div>
